...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        grid=[[' ' for i in range(0,self.width)] for j in range(0,self.height)]
        for item in self.walls:
            grid[item[0]][item[1]]='%'
        for item in self.foods:
            grid[item[0]][item[1]]='.'
        grid[self.pacman_pos[0]][self.pacman_pos[1]]='P'
...
def q6(file_path):
    ...
    '''
    la=Layout.build_layout(file_path)
