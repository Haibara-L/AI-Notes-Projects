    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            for action in action_List:
                ...
                tempvalue=value
                value=max(value,minimizer(newstate,depth,1)[0])
                if value>tempvalue:
                    maxiAction=action
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            action_list=state.getLegalActions(index_of_agent)
            for action in action_list:
                tempvalue=value
                newstate=state.generateSuccessor(index_of_agent,action)
                if index_of_agent==agentnum:
                    value=min(value,maximizer(newstate,depth-1,0)[0])
                    if value<tempvalue:
                        miniAction=action
                else:
                    value=min(value,minimizer(newstate,depth,index_of_agent+1)[0])
                    if value<tempvalue:
                        miniAction=action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in action_list:
                newstate=state.generateSuccessor(0,action)
                cur_value=minimizer(newstate,depth,1,alpha,beta)[0]
                if(cur_value>value):
                    maxiAction=action
                    value=cur_value
                if value>beta:
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in action_list:
                newstate=state.generateSuccessor(index_of_agent,action)
                if index_of_agent==agentnum:
                    cur_value=maximizer(newstate,depth-1,0,alpha,beta)[0]
                    if cur_value<value:
                        miniAction=action
                        value=cur_value
                    if value<alpha:
                    ...
                    cur_value=minimizer(newstate,depth,index_of_agent+1,alpha,beta)[0]
                    if cur_value<value:
                        miniAction=action
                        value=cur_value
                    if value<alpha:
...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        def Selection(cgs, cgstree):
            ...
            '''
            while 1:
                if cgstree.statevalue.isWin() or cgstree.statevalue.isLose():
                    return (cgstree.statevalue, cgstree)
                action_list=cgstree.statevalue.getLegalActions(0)
                actions = [a for a in action_list if a != 'Stop']
                if not actions:
                    return (cgstree.statevalue, cgstree)
                action_list=actions
                expanded_children=[]
                for action in action_list:
                    if getattr(cgstree,action.lower())==None:
                        return (cgs,cgstree)
                    expanded_children.append((getattr(cgstree,action.lower()),action))
                cgs,bestaction=best_UCT(expanded_children)
                cgstree=getattr(cgstree,bestaction.lower())
            return (cgs, cgstree)
        ...
        def Expansion(cgstree):
            ...
            for action in legal_actions:
                newstate=cgstree.statevalue.generateSuccessor(0,action)
                child_node=Node([newstate,0,1])
                child_node.parent=cgstree
                if action=="North":
                    cgstree.north=child_node
                elif action=="South":
                    cgstree.south=child_node
                elif action=="West":
                    cgstree.west=child_node
                elif action=="East":
                    cgstree.east=child_node
                else:
                    cgstree.stop=child_node
